package nl.jorisdormans.phantomextra.editor 
{
	...
	import nl.jorisdormans.phantomextra.gui.*;
	/**
	 * ...
	...
	public class Editor extends Screen
	{
		...
		public function Editor() 
		{
			...
			scaleFactor = (this.screenHeight - 4*28 - 8)/this.screenHeight;
			layerPanelWidth = this.screenWidth;
			layerPanelHeight = (1 - scaleFactor) * this.screenHeight;
			toolPanelWidth = (1 - scaleFactor) * this.screenWidth;
			toolPanelHeight = scaleFactor * this.screenHeight;
			
			toolBorder = new PhantomBorder(this.sprite, this.screenWidth * scaleFactor, 0, toolPanelWidth, toolPanelHeight);
			layerBorder = new PhantomBorder(this.sprite, 0, this.screenHeight * scaleFactor, layerPanelWidth, layerPanelHeight);
			buttonPanel = new PhantomPanel(layerBorder, this.screenWidth * scaleFactor + 2, 2, toolPanelWidth - 4, layerPanelHeight - 4);
		...
		{
			if (e.target is PhantomControl || e.target is TextField) return;
				...
				switch (currentData.editMode) {
					...
					case LayerData.EDIT_TILES:
						...
						if (toolBox.selectedIndex>=0) {
							...
							var c:XML = ContentManager.getInstance().getTile(toolBox.selectedOption);
							if (c) {
								var p:Vector3D = new Vector3D();
								hoverObject = ObjectFactory.getInstance().generateFromXML(c);
								...
								tl.addTileObject(hoverObject, p);
							}
						}
						break;
			...
			switch (currentData.editMode) {
				...
				case LayerData.EDIT_TILES:
					...
					if (tl) {
						mousePositionX = e.stageX / scaleFactor;
						mousePositionY = e.stageY / scaleFactor;
						mousePositionX += currentLayer.screen.camera.left;
						mousePositionY += currentLayer.screen.camera.top;
						...
						hoverObject = tl.getTileObject(mousePositionX, mousePositionY);
						...
						drawX -= currentLayer.screen.camera.left * scaleFactor;
						drawY -= currentLayer.screen.camera.top * scaleFactor;
					}					
				...
				case LayerData.EDIT_OBJECTS:
					...
					if (ol) {
						...
						if (e.buttonDown && selectedObject) {
							...
							hoverObject = ol.getObjectAt(new Vector3D(mousePositionX, mousePositionY), null, true);
						}
						
						if (controlKey) {
							drawX = (mousePositionX - currentLayer.screen.camera.left) * scaleFactor;
							drawY = (mousePositionY - currentLayer.screen.camera.top) * scaleFactor;
						} else {
							drawX = e.stageX;
							drawY = e.stageY;
						}
		...
		override public function render(camera:Camera):void 
		{
			...
			switch (currentData.editMode) {
				case LayerData.EDIT_CURVES:
					...
					dx = mousePositionX - currentLayer.gameScreen.camera.left;
					dy = mousePositionY - currentLayer.gameScreen.camera.top;
					dx *= scaleFactor;
					dy *= scaleFactor;
				...
				case LayerData.EDIT_OBJECTS:
					if (hoverObject && hoverObject.shape) {
						dx = hoverObject.position.x - currentLayer.screen.camera.left;
						dy = hoverObject.position.y - currentLayer.screen.camera.top;
						dx *= scaleFactor;
						dy *= scaleFactor;
						sprite.graphics.lineStyle(3, 0x000000, 0.5);
					...
					if (selectedObject && selectedObject.shape) {
						dx = selectedObject.position.x - currentLayer.screen.camera.left;
						dy = selectedObject.position.y - currentLayer.screen.camera.top;
						dx *= scaleFactor;
						dy *= scaleFactor;
						sprite.graphics.lineStyle(3, 0x000000, 0.5);
			...
			if (currentLayer is TiledObjectLayer) {
				...
				dx = -(currentLayer.screen.camera.left % ts); 
				while (dx < currentLayer.screen.screenWidth) {
					sprite.graphics.moveTo(dx * scaleFactor, 0)
					sprite.graphics.lineTo(dx * scaleFactor, currentLayer.screen.screenHeight * scaleFactor);
					dx += ts;
				}
				
				dy = -(currentLayer.screen.camera.top % ts); 
				while (dy < currentLayer.screen.screenHeight) {
					sprite.graphics.moveTo(0, dy * scaleFactor)
					sprite.graphics.lineTo(currentLayer.screen.screenWidth* scaleFactor, dy * scaleFactor);
					dy += ts;
				}
				sprite.graphics.lineStyle();
		...
		{
			super.activate();
			PhantomGame.log("Activating editor...", "Info", "Phantom");
			...
			if (screenBelow) {
				...
				for (var i:int = 0; i < screenBelow.components.length; i++) {
					if (screenBelow.components[i] is TiledObjectLayer) {
					...
					if (screenBelow.components[i] is ObjectLayer) {
		...
		private function onKeyDown(e:KeyboardEvent):void 
		{
			...
			//if (e.keyCode == Keyboard.LEFT && currentLayer) currentLayer.screen.camera.moveCamera( -cameraMoveDistance, 0);
			//if (e.keyCode == Keyboard.RIGHT && currentLayer) currentLayer.screen.camera.moveCamera(cameraMoveDistance, 0);
			//if (e.keyCode == Keyboard.UP && currentLayer) currentLayer.screen.camera.moveCamera(0, -cameraMoveDistance);
			//if (e.keyCode == Keyboard.DOWN && currentLayer) currentLayer.screen.camera.moveCamera(0, cameraMoveDistance);
			...
			if (e.keyCode == 219) {
				if (toolObject) {
					toolObject.shape.orientation -= 5 * MathUtil.TO_RADIANS;
				}
				if (selectedObject) {
					selectedObject.shape.orientation -= -5 * MathUtil.TO_RADIANS;
				}
			}
			if (e.keyCode == 221) {
				if (toolObject) {
		...
		override public function deactivate():void 
		{
			...
			super.deactivate();
			PhantomGame.log("Deactivating editor...", "Info", "Phantom");
		...
		private function setCurrentLayer(layerDataItem:LayerData):void {
			...
			if (currentData.editMode == LayerData.EDIT_OBJECTS && currentLayer is ObjectLayer) {
				...
				for (var key:String in ContentManager.getInstance().objects) {
					toolBox.addOption(key);
				}
			}
			
			if (currentData.editMode == LayerData.EDIT_TILES && currentLayer is TiledObjectLayer) {
				toolBox.addOption("<empty>");
				for (key in ContentManager.getInstance().tiles) {
					toolBox.addOption(key);
				}
			}
		...
		private function changeTool(sender:PhantomSelectBox):void {
			...
			if (currentData.editMode == LayerData.EDIT_OBJECTS && currentLayer is ObjectLayer) {
				var c:XML = ContentManager.getInstance().getObject(toolBox.selectedOption);
				if (c) {
					toolObject = ObjectFactory.getInstance().generateFromXML(c);
				} else {
					toolObject = null;
				}
			}
			if (currentData.editMode == LayerData.EDIT_TILES && currentLayer is TiledObjectLayer) {
				c = ContentManager.getInstance().getTile(toolBox.selectedOption);
				if (c) {
					toolObject = ObjectFactory.getInstance().generateFromXML(c);
				} else {
					toolObject = null;
		...
		private function changeProperties(dialog:PhantomDialog):void {
			if (selectedObject) {
				...
				//TODO: check for changed sort orders
			}
		}
